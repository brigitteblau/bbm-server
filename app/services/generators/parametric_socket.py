...
class SocketProfile:
    ...
    def from_params(cls, params: SocketParameters) -> "SocketProfile":
        ...
        return cls(
            ...
            wall_height=height - dome_height,
...
class VentPattern:
    ...
    def is_hole(self, angle_deg: float, t: float) -> bool:
        if self.donning_gap_deg > 0.0:
            delta = abs(_wrap_deg(angle_deg - 180.0))
            if delta <= self.donning_gap_deg / 2.0 and t >= 0.18:
                return True

        if self.columns <= 0 or self.rows <= 0:
            return False
        if not (VENT_BAND[0] <= t <= VENT_BAND[1]):
            return False
        ...
        for row in range(self.rows):
            ...
            for column in range(self.columns):
                a_center = column_step * column + offset
                du = _wrap_deg(angle_deg - a_center) / self.angular_half_deg
                dv = (t - t_center) / self.height_half
                if abs(du) >= 1.0 or abs(dv) >= 1.0:
                    continue
                # Superelipse: slot rectangular de puntas redondeadas.
                if abs(du) ** VENT_ROUNDNESS + abs(dv) ** VENT_ROUNDNESS <= 1.0:
                    return True
        return False
...
def _oriented_quad(mesh: Mesh, a: int, b: int, c: int, d: int, reference) -> None:
    """Agrega el quad con el sentido que deja la normal apuntando como reference."""
    normal = _face_normal(mesh, a, b, c)
    if _dot(normal, reference) < 0.0:
...
def build_socket_mesh(
    ...
    *,
    ...
    # Anillos de la pared (exterior e interior).
    ...
    for k in range(nv + 1):
        ...
        outer_rings.append(_ring(mesh, r_out, z, nu, ellipse_ratio))
        inner_rings.append(_ring(mesh, r_out - profile.wall, z, nu, ellipse_ratio))

    # Máscara de material: celda (i, k) sólida o agujero.
    solid = [[True] * nv for _ in range(nu)]
    for i in range(nu):
        angle = 360.0 * (i + 0.5) / nu
        for k in range(nv):
            t = (k + 0.5) / nv
            solid[i][k] = not pattern.is_hole(angle, t)
    ...
    for i in range(nu):
        ...
        for k in range(nv):
            ...
            cell_center = _centroid(mesh, (o_lo[i], o_hi[j], i_lo[i], i_hi[j]))

            # Costura del canto contra cada vecino que sea agujero.
            if not is_solid(i, k + 1):     # canto superior
                edge = (o_hi[i], o_hi[j], i_hi[j], i_hi[i])
                _oriented_quad(mesh, *edge, _sub(_centroid(mesh, edge), cell_center))
            if not is_solid(i, k - 1):     # canto inferior
                edge = (o_lo[i], o_lo[j], i_lo[j], i_lo[i])
                _oriented_quad(mesh, *edge, _sub(_centroid(mesh, edge), cell_center))
            if not is_solid(i + 1, k):     # canto lateral +θ
                edge = (o_lo[j], o_hi[j], i_hi[j], i_lo[j])
                _oriented_quad(mesh, *edge, _sub(_centroid(mesh, edge), cell_center))
            if not is_solid(i - 1, k):     # canto lateral -θ
                edge = (o_lo[i], o_hi[i], i_hi[i], i_lo[i])
                _oriented_quad(mesh, *edge, _sub(_centroid(mesh, edge), cell_center))
